...
(
    ...
    CountryCode NCHAR(2) NOT NULL CONSTRAINT UQ_Country_Code UNIQUE,
...
(
    ...
    CONSTRAINT FK_Region_Country FOREIGN KEY (CountryID) REFERENCES Core.Country(CountryID)
);
GO
...
(
    ...
    PermissionCode NVARCHAR(80) NOT NULL CONSTRAINT UQ_Permission_Code UNIQUE,
...
(
    ...
    CONSTRAINT FK_RolePermission_Role FOREIGN KEY (RoleID) REFERENCES [Identity].Role(RoleID),
    CONSTRAINT FK_RolePermission_Permission FOREIGN KEY (PermissionID) REFERENCES [Identity].Permission(PermissionID)
);
GO
...
CREATE TABLE [Identity].CustomerProfile
(
    CustomerID INT IDENTITY(1,1) NOT NULL CONSTRAINT PK_CustomerProfile PRIMARY KEY,
    UserID INT NOT NULL CONSTRAINT UQ_CustomerProfile_User UNIQUE,
    LoyaltyLevel NVARCHAR(20) NOT NULL DEFAULT N'Standard',
    TotalRewardPoints INT NOT NULL DEFAULT 0,
    CreatedAt DATETIME2 NOT NULL DEFAULT SYSDATETIME(),
    CONSTRAINT FK_CustomerProfile_User FOREIGN KEY (UserID) REFERENCES [Identity].UserAccount(UserID),
    CONSTRAINT CK_CustomerProfile_Level CHECK (LoyaltyLevel IN (N'Standard', N'Silver', N'Gold', N'Platinum'))
);
GO

CREATE TABLE [Identity].StaffProfile
(
    StaffID INT IDENTITY(1,1) NOT NULL CONSTRAINT PK_StaffProfile PRIMARY KEY,
    UserID INT NOT NULL CONSTRAINT UQ_StaffProfile_User UNIQUE,
    EmployeeCode NVARCHAR(30) NOT NULL CONSTRAINT UQ_StaffProfile_Code UNIQUE,
    Department NVARCHAR(80) NOT NULL,
    ManagedRegionID INT NULL,
    CreatedAt DATETIME2 NOT NULL DEFAULT SYSDATETIME(),
    CONSTRAINT FK_StaffProfile_User FOREIGN KEY (UserID) REFERENCES [Identity].UserAccount(UserID),
    CONSTRAINT FK_StaffProfile_Region FOREIGN KEY (ManagedRegionID) REFERENCES Core.Region(RegionID)
);
GO
...
(
    ...
    MethodCode NVARCHAR(30) NOT NULL CONSTRAINT UQ_PaymentMethod_Code UNIQUE,
...
(
    ...
    UserID INT NOT NULL CONSTRAINT UQ_Wallet_User UNIQUE,
    WalletCode NVARCHAR(40) NOT NULL CONSTRAINT UQ_Wallet_Code UNIQUE,
    Balance DECIMAL(19,4) NOT NULL DEFAULT 0,
    CurrencyCode NCHAR(3) NOT NULL DEFAULT N'VND',
    IsActive BIT NOT NULL DEFAULT 1,
    CreatedAt DATETIME2 NOT NULL DEFAULT SYSDATETIME(),
    LastTransactionAt DATETIME2 NULL,
    CONSTRAINT FK_Wallet_User FOREIGN KEY (UserID) REFERENCES [Identity].UserAccount(UserID),
...
(
    ...
    CONSTRAINT FK_PaymentTransaction_Method FOREIGN KEY (PaymentMethodID) REFERENCES Payments.PaymentMethod(PaymentMethodID),
    CONSTRAINT CK_PaymentTransaction_Amount CHECK (Amount >= 0),
    CONSTRAINT CK_PaymentTransaction_Direction CHECK (Direction IN (N'D', N'C')),
    CONSTRAINT CK_PaymentTransaction_Type CHECK (TransactionType IN (N'ChargingPayment', N'WalletTopUp', N'Refund', N'SettlementPayout')),
    CONSTRAINT CK_PaymentTransaction_Status CHECK (TransactionStatus IN (N'Pending', N'Completed', N'Failed', N'Refunded', N'Cancelled'))
);
GO
...
(
    ...
    Description NVARCHAR(500) NULL,
    CreatedAt DATETIME2 NOT NULL DEFAULT SYSDATETIME(),
    CONSTRAINT FK_WalletTransaction_Wallet FOREIGN KEY (WalletID) REFERENCES Payments.Wallet(WalletID),
    CONSTRAINT FK_WalletTransaction_Transaction FOREIGN KEY (TransactionID) REFERENCES Payments.PaymentTransaction(TransactionID),
    CONSTRAINT CK_WalletTransaction_Direction CHECK (Direction IN (N'D', N'C'))
);
GO

CREATE TABLE Payments.QRPaymentRequest
(
    QRRequestID BIGINT IDENTITY(1,1) NOT NULL CONSTRAINT PK_QRPaymentRequest PRIMARY KEY,
    RequestCode NVARCHAR(40) NOT NULL CONSTRAINT UQ_QRPaymentRequest_Code UNIQUE,
    UserID INT NOT NULL,
    SessionID BIGINT NULL,
    Amount DECIMAL(19,4) NOT NULL,
    QRPayload NVARCHAR(1000) NOT NULL,
    RequestStatus NVARCHAR(20) NOT NULL DEFAULT N'Pending',
    ExpiresAt DATETIME2 NOT NULL,
    TransactionID BIGINT NULL,
    CreatedAt DATETIME2 NOT NULL DEFAULT SYSDATETIME(),
    CONSTRAINT FK_QRPaymentRequest_User FOREIGN KEY (UserID) REFERENCES [Identity].UserAccount(UserID),
    CONSTRAINT FK_QRPaymentRequest_Session FOREIGN KEY (SessionID) REFERENCES Operations.ChargingSession(SessionID),
    CONSTRAINT FK_QRPaymentRequest_Transaction FOREIGN KEY (TransactionID) REFERENCES Payments.PaymentTransaction(TransactionID),
    CONSTRAINT CK_QRPaymentRequest_Amount CHECK (Amount > 0),
    CONSTRAINT CK_QRPaymentRequest_Status CHECK (RequestStatus IN (N'Pending', N'Paid', N'Expired', N'Cancelled'))
);
GO
...
(
    ...
    RefundCode NVARCHAR(40) NOT NULL CONSTRAINT UQ_Refund_Code UNIQUE,
    ...
    CONSTRAINT FK_Refund_OriginalTransaction FOREIGN KEY (OriginalTransactionID) REFERENCES Payments.PaymentTransaction(TransactionID),
    CONSTRAINT FK_Refund_RefundTransaction FOREIGN KEY (RefundTransactionID) REFERENCES Payments.PaymentTransaction(TransactionID),
    CONSTRAINT FK_Refund_User FOREIGN KEY (UserID) REFERENCES [Identity].UserAccount(UserID),
    CONSTRAINT CK_Refund_Amount CHECK (Amount > 0),
    CONSTRAINT CK_Refund_Status CHECK (RefundStatus IN (N'Pending', N'Completed', N'Failed', N'Cancelled'))
);
GO
...
CREATE TABLE Maintenance.ErrorCatalog
(
    ErrorCode NVARCHAR(30) NOT NULL CONSTRAINT PK_ErrorCatalog PRIMARY KEY,
    ErrorName NVARCHAR(150) NOT NULL,
    DefaultSeverity NVARCHAR(20) NOT NULL,
    Description NVARCHAR(500) NULL,
    CONSTRAINT CK_ErrorCatalog_Severity CHECK (DefaultSeverity IN (N'Low', N'Medium', N'High', N'Critical'))
);
GO
...
(
    ...
    ResolvedBy INT NULL,
    IsActive BIT NOT NULL DEFAULT 1,
    CONSTRAINT FK_ErrorLog_Catalog FOREIGN KEY (ErrorCode) REFERENCES Maintenance.ErrorCatalog(ErrorCode),
...
CREATE TABLE Maintenance.MaintenanceAssignment
(
    AssignmentID BIGINT IDENTITY(1,1) NOT NULL CONSTRAINT PK_MaintenanceAssignment PRIMARY KEY,
    TicketID BIGINT NOT NULL,
    TechnicianUserID INT NOT NULL,
    AssignedBy INT NOT NULL,
    AssignedAt DATETIME2 NOT NULL DEFAULT SYSDATETIME(),
    CONSTRAINT FK_MaintenanceAssignment_Ticket FOREIGN KEY (TicketID) REFERENCES Maintenance.MaintenanceTicket(TicketID),
    CONSTRAINT FK_MaintenanceAssignment_Technician FOREIGN KEY (TechnicianUserID) REFERENCES [Identity].UserAccount(UserID),
    CONSTRAINT FK_MaintenanceAssignment_AssignedBy FOREIGN KEY (AssignedBy) REFERENCES [Identity].UserAccount(UserID)
);
GO

CREATE TABLE Maintenance.MaintenanceHistory
(
    HistoryID BIGINT IDENTITY(1,1) NOT NULL CONSTRAINT PK_MaintenanceHistory PRIMARY KEY,
    TicketID BIGINT NOT NULL,
    ...
    ChangedAt DATETIME2 NOT NULL DEFAULT SYSDATETIME(),
    CONSTRAINT FK_MaintenanceHistory_Ticket FOREIGN KEY (TicketID) REFERENCES Maintenance.MaintenanceTicket(TicketID),
    CONSTRAINT FK_MaintenanceHistory_User FOREIGN KEY (ChangedBy) REFERENCES [Identity].UserAccount(UserID)
);
GO
...
(
    ...
    ChangedAt DATETIME2 NOT NULL DEFAULT SYSDATETIME(),
    CONSTRAINT CK_AuditLog_Action CHECK (ActionType IN (N'INSERT', N'UPDATE', N'DELETE', N'PAYMENT', N'REFUND', N'SETTLEMENT', N'SECURITY'))
);
GO

PRINT N'02 - Tables created.';
GO
